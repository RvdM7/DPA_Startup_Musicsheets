...
namespace DPA_Musicsheets.Refactoring
{
    class ConvertToPSAM : IConverter<ISymbol>
    {
        object IConverter<ISymbol>.convert(List<ISymbol> symbols)
        {
            ...
            musicalSymbols.Add(new PSAMControlLibrary.Clef(ClefType.GClef, 2));
            foreach (ISymbol symbol in symbols)
            {
                if (symbol is Domain.Note)
                {
                    Domain.Note note = (Domain.Note)symbol;
                    ...
                    var PSAMNote = new PSAMControlLibrary.Note(note.height.ToString().ToUpper(), alter, relativeOctave, (MusicalSymbolDuration)noteLength, NoteStemDirection.Up, NoteTieType.None, new List<NoteBeamType>() { NoteBeamType.Single });
